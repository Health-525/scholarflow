...
| 能力 | Electron 桌面端 | Web / PWA | Android / Capacitor | iOS / Capacitor |
| --- | --- | --- | --- | --- |
| 课表 / 成绩 / 考试同步 | 支持 | 支持 | 实验性 | 实验性 |
| 作业 / 目标 / 番茄钟 / 笔记 | 支持 | 支持 | 实验性 | 实验性 |
| 图书馆预约与 JWT 刷新 | 支持更完整 | 受浏览器限制 | 实验性 | 实验性 |
| 本地安全加密存储 | 支持 | 不完整 | 不完整 | 不完整 |
| 活动窗口统计 | 支持 | 不支持 | 不支持 | 不支持 |
| 后台自动刷新 | 支持 | 不支持 | 不支持 | 不支持 |
| 萌系移动端皮肤 | 桌面端 UI | 自适应 | 萌系 UI | 萌系 UI |
| 端侧 AI (MNN) | 不支持 | 不支持 | 开发中 | 支持 |
