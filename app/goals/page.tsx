...
export default function DailyGoalsPage() {
      ...
      try {
        ...
        if (state.date && state.date !== today) {
          if (nextGoals.length > 0) {
            ...
            const record: HistoryRecord = { date: state.date, completed, total: nextGoals.length };
  ...
  const week = useMemo(() => {
    const days: { label: string; date: Date; full: boolean; hasData: boolean }[] = [];
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      days.push({
        ...
        date: d,
        full: record ? record.completed === record.total && record.total > 0 : false,
        hasData: !!record,
      });
    }
    return days;
    ...
    <div className="max-w-3xl mx-auto py-6 animate-page">
            ...
            {week.map((d, i) => {
              ...
              return (
                <div key={i} className="text-center">
                  ...
                  <div className="text-[10px] mt-1 text-muted-foreground">{d.label}</div>
                </div>
              );
            })}
          </div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <Card className="hover:shadow-sm hover:translate-y-0">
          <CardContent className="flex items-center gap-5 py-5">
            ...
            <div>
              <div className="text-[11px] font-semibold text-muted-foreground mb-0.5">今日进度</div>
              <div className="text-2xl font-bold tabular-nums text-foreground">{done}/{goals.length}</div>
              <div className="text-[10px] text-muted-foreground">{goals.length === 0 ? "先添加目标" : allDone ? "全部完成" : "继续加油"}</div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="hover:shadow-sm hover:translate-y-0">
          <CardContent className="flex items-center gap-5 py-5">
            ...
            <div>
              <div className="text-[11px] font-semibold text-muted-foreground mb-0.5">连续天数</div>
              <div className="text-2xl font-bold tabular-nums text-foreground">{loaded ? streak : "—"}</div>
              <div className="text-[10px] text-muted-foreground">{streak > 0 ? "保持连胜" : "从全部完成开始"}</div>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      {allDone && (
        <Card className="mb-5 border-green-500/20 dark:border-green-500/30 bg-gradient-to-r from-green-500/10 to-emerald-500/5 dark:from-green-500/15 dark:to-emerald-500/10 animate-fade-up hover:shadow-sm hover:translate-y-0">
          <CardContent className="py-5 text-center">
            ...
            <div className="text-[15px] font-semibold text-green-600 dark:text-green-400">今日目标全部达成！</div>
            <div className="text-[11px] text-muted-foreground mt-1">
              {streak > 0 ? `连续 ${streak} 天，明天继续` : "明天继续设定新目标"}
            </div>
          </CardContent>
        </Card>
      )}
      ...
      <Card className="mb-4 hover:shadow-sm hover:translate-y-0">
        <CardContent className="py-4">
          <label htmlFor="new-goal" className="block text-[12px] font-medium text-muted-foreground mb-1.5">
            今天要做什么？
          </label>
          <div className="flex items-center gap-2">
            ...
            />
            <Button onClick={add} disabled={!newGoal.trim()} className="h-11 w-12 shrink-0 p-0 rounded-xl">
      ...
      {deletedBuffer && Date.now() < deletedBuffer.expiresAt && (
        <div className="flex items-center gap-2 px-3 py-2.5 rounded-xl text-sm bg-emerald-500/10 dark:bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 animate-fade-up mb-4">
          <span className="flex-1 truncate">已删除「{deletedBuffer.goal.text}」</span>
          <Button variant="secondary" size="sm" onClick={undoDelete} className="gap-1">
            <Trash2 size={12} /> 撤销
          </Button>
        </div>
      )}
              ...
              {goals.map((g) => (
                <div
                  ...
                  className={`w-full text-left flex items-center gap-3 p-3 rounded-xl transition-colors ${
                    g.done ? "bg-green-500/5 dark:bg-green-500/10" : "hover:bg-muted/40"
                  }`}
                >
                  <button
                    type="button"
