...
const levelLabel = (level: number) =>
  level >= 80 ? "精通" : level >= 60 ? "熟练" : level >= 30 ? "入门" : "空白";
...
export default function KnowledgeProfilePage() {
  ...
  const knowledgeGaps = [
    {
      type: "桥接型",
      title: "协方差矩阵→特征值→主成分",
      detail: "已学特征值分解 + 已学PCA调用，缺少完整推导",
      priority: 1,
    },
    {
      type: "深度不足",
      title: "神经网络训练循环",
      detail: "会用numpy/sklearn，缺少从零实现训练循环",
      priority: 2,
    },
    {
      type: "缺失模块",
      title: "Transformer注意力机制",
      detail: "AI学习目录几乎为空，建议从注意力机制开始",
      priority: 1,
    },
    {
      type: "兴趣信号",
      title: "MCP协议规范",
      detail: "多次提到MCP但未系统学习协议规范",
      priority: 3,
    },
  ];
      ...
      <div className="max-w-5xl mx-auto py-6 animate-page">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-[12px] text-muted-foreground">
              综合水平 {avgLevel}% · 优势 {strengths.length} 项 · 空白{" "}
              {gaps.length} 项
            </p>
          </div>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
          {[1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className="rounded-2xl p-4 bg-card border border-border skeleton h-24"
            />
          ))}
        </div>
  ...
  return (
    <div className="max-w-5xl mx-auto py-6 animate-page">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <div className="rounded-2xl p-4 bg-card border border-[var(--status-success)]/20 shadow-sm animate-fade-up stagger-1">
          <div className="flex items-center gap-2 mb-2">
            <TrendingUp className="w-3.5 h-3.5 text-[var(--status-success)]" />
            <span className="text-[10px] font-semibold text-muted-foreground">
              优势领域
            </span>
          </div>
          <div className="text-2xl font-bold tabular-nums text-[var(--status-success)] animate-count">
            {strengths.length}
          </div>
        ...
        <div className="rounded-2xl p-4 bg-card border border-primary/20 shadow-sm animate-fade-up stagger-2">
          ...
          <div className="text-2xl font-bold tabular-nums text-primary animate-count">
            {avgLevel}%
          </div>
          <div className="text-[10px] text-muted-foreground">14项技能均值</div>
        </div>
        <div className="rounded-2xl p-4 bg-card border border-destructive/20 shadow-sm animate-fade-up stagger-3">
          <div className="flex items-center gap-2 mb-2">
          ...
          <div className="text-2xl font-bold tabular-nums text-destructive animate-count">
            {gaps.length}
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        ...
        <div className="rounded-2xl p-5 bg-card border border-border shadow-sm animate-fade-up stagger-5">
          <h2 className="text-[13px] font-semibold mb-3 text-foreground">
            技能雷达图
          </h2>
                ...
                <div className="space-y-2">
                  {catSkills.map((skill) => (
                    <div key={skill.name} className="flex items-center gap-2">
                      <span className="text-[11px] text-muted-foreground w-[100px] truncate">
                        {skill.name}
                      </span>
                      <div className="flex-1 h-1.5 rounded-full bg-secondary overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${skill.level}%`,
                            backgroundColor: levelColor(skill.level, isDark),
                          }}
                        />
                      </div>
                      <span
                        className="text-[10px] font-mono tabular-nums shrink-0"
                        style={{ color: levelColor(skill.level, isDark) }}
                      >
                        {skill.level}%
                      </span>
                    </div>
      ...
      <div className="rounded-2xl p-5 bg-card border border-border shadow-sm animate-fade-up stagger-7 mb-6">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {GAP_TYPES.map((gt) => (
            <div
              key={gt.type}
              className="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-secondary"
            >
              ...
              >
                ...
                <div className="mt-2 flex items-center gap-1.5">
                  <span
                    ...
                    style={{
                      backgroundColor: `${gapMeta.color}10`,
                      color: gapMeta.color,
                    }}
                  >
                    {gap.type}
      ...
      <div className="rounded-2xl p-5 bg-card border border-border shadow-sm animate-fade-up stagger-8">
        <h2 className="text-[13px] font-semibold mb-4 text-foreground">
          技术栈全景
        </h2>
                  ...
                  <div className="flex justify-between text-[12px] mb-1">
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      <span
                        className="text-[10px] px-1.5 py-0.5 rounded font-medium"
                        style={{
                          backgroundColor: `${levelColor(skill.level, isDark)}10`,
                          color: levelColor(skill.level, isDark),
                        }}
                      >
