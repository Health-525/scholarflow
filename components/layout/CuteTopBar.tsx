...
export function CuteTopBar() {
  return (
    <header
      className="md:hidden sticky top-0 z-30 flex items-center justify-between px-4 pb-2.5 bg-surface/80 backdrop-blur-md border-b border-border/50"
      style={{ paddingTop: "calc(env(safe-area-inset-top) + 0.625rem)" }}
    >
      <Link href="/" className="flex items-center gap-3 active:scale-95 transition-transform">
        <span className="flex h-10 w-10 items-center justify-center rounded-full bg-surface-container shadow-sm">
        ...
        <h1 className="text-2xl font-bold tracking-tight text-primary">小咪学习助手</h1>
      </Link>
      <Link
        href="/settings"
        aria-label="设置"
