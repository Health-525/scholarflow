...
import { getWeekNumber, type RawScheduleData } from "@/lib/schedule/schedule";
...
interface ScheduleClientProps {
  initialSchedule: RawScheduleData | null;
  initialAdjustments: Adjustment[];
}

export function ScheduleClient({ initialSchedule, initialAdjustments }: ScheduleClientProps) {
  const isMobile = useIsMobile();
  const [activeTab, setActiveTab] = useState<Tab>("week");
  const { data, isLoading, error, refetch } = useScheduleQuery(
    initialSchedule ? { schedule: initialSchedule, adjustments: initialAdjustments } : undefined
  );
