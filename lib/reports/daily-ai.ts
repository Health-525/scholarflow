...
export function buildDailyReportPrompt(input: DailyReportInput): string {
  ...
  lines.push("【输出要求】");
  lines.push("1. 使用 Markdown 格式。主标题必须根据当天最核心的事件或状态生成一个简短主题（5-12 字），例如：`# 无课日的屏幕时间反思`、`# 备考冲刺：距高数考试还有 2 天`、`# 作业截止日：3 项待提交`、`# 高效专注日：6 个番茄钟`、`# 考试日：数据结构`。不要只用 `# 日报` 这种泛泛标题；");
  lines.push("2. 主标题下方用一行引用（`> 6月23日 周二`）标注日期，不要重复在标题里写日期；");
  lines.push("3. 包含以下几个小节：今日概览、课程回顾、作业进展、考试/目标/运动/专注（如适用）、收获与反思、明日计划；");
