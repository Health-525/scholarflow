...
// ── Generic semantic color helpers（图书馆组件使用）──
...
/**
 * 座位占用率 → 语义色。入参是**占用率**（used / total，越高越拥挤），不是空闲率。
 *
 * 函数名刻意点明口径：调用方曾各传一种（阅览室卡片传占用率、汇总卡片传空闲率），
 * 同一函数对「数值高」的含义相反，两边必有一边配色是反的。这里用命名约束入参语义。
 * 色值一律取自 semanticColor，暗色模式与主题配色只保留一份来源。
 */
export function occupancyColor(usedPercent: number): string {
  const pct = Number.isFinite(usedPercent)
    ? Math.min(100, Math.max(0, usedPercent))
    : 0;
  if (pct >= 85) return semanticColor("error"); // 几乎满座
  if (pct >= 50) return semanticColor("warning"); // 偏紧
  return semanticColor("success"); // 宽松
}
